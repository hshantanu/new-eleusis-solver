...
def scan_and_rank_hypothesis(three_length_hypothesis_flag):
            ...
            for characteristic_tuple in itertools.product(*combined_char_indices_list):
                ...
                if (or_tuple[0] in combined_char_indices_list[0] or or_tuple[1] in combined_char_indices_list[1] or or_tuple[2] in combined_char_indices_list[2]):
                    if or_characterstic_tuple in hypothesis_dict.keys():
                        hypothesis_occurrence_count[or_characterstic_tuple] += 1
                        hypothesis_dict[or_characterstic_tuple] += ((weighted_property_dict[or_tuple[0]]+weighted_property_dict[or_tuple[1]] + weighted_property_dict[or_tuple[2]])/2)*hypothesis_occurrence_count[or_tuple]
                    else:
                        hypothesis_occurrence_count[or_characterstic_tuple] = 1
                        hypothesis_dict[or_characterstic_tuple] = (weighted_property_dict[or_tuple[0]]+weighted_property_dict[or_tuple[1]]+weighted_property_dict[or_tuple[2]])/2
                mean += (weighted_property_dict[or_tuple[0]]+weighted_property_dict[or_tuple[1]]+weighted_property_dict[or_tuple[2]])/3
                # and logic
    ...
    for i in xrange(1, len(legal_cards)):
        ...
        for characteristic_tuple in itertools.product(*combined_char_indices_list):
            ...
            if (or_tuple[0] in combined_char_indices_list[0] or or_tuple[1] in combined_char_indices_list[1]):
                if or_characterstic_tuple in hypothesis_dict.keys():
                    hypothesis_occurrence_count[or_characterstic_tuple] += 1
                    hypothesis_dict[or_characterstic_tuple] += ((weighted_property_dict[or_tuple[0]]+weighted_property_dict[or_tuple[1]])/2)*hypothesis_occurrence_count[or_tuple]
                else:
                    hypothesis_occurrence_count[or_characterstic_tuple] = 1
                    hypothesis_dict[or_characterstic_tuple] = (weighted_property_dict[or_tuple[0]]+weighted_property_dict[or_tuple[1]])/2
            mean += (weighted_property_dict[or_tuple[0]]+weighted_property_dict[or_tuple[1]])/3
            # and logic
            if characteristic_tuple in hypothesis_dict.keys():
                hypothesis_occurrence_count[characteristic_tuple] += 1
    ...
    if hypothesis_offset == 0:
        hypothesis_offset = 1 
    print('-------------------hypothesis_offset-----------------------', hypothesis_offset)
            ...
            for characteristic_tuple in itertools.product(*combined_char_indices_list):
                ...
                if (or_tuple[0] not in combined_char_indices_list[0] and or_tuple[1] not in combined_char_indices_list[1] and or_tuple[2] not in combined_char_indices_list[2]):
                        if or_characterstic_tuple in ranked_hypothesis.keys():
                            if or_characterstic_tuple in hypothesis_occurrence_count:
                                hypothesis_occurrence_count[or_characterstic_tuple] += 1
                            else:
                                hypothesis_occurrence_count[or_characterstic_tuple] = 1
                            ranked_hypothesis[or_characterstic_tuple] -= ((weighted_property_dict[or_tuple[0]]+weighted_property_dict[or_tuple[1]] + weighted_property_dict[or_tuple[2]])/3)*hypothesis_occurrence_count[or_characterstic_tuple]
                    ...
                    if (or_tuple[0] not in combined_char_indices_list[0] and or_tuple[1] not in combined_char_indices_list[1]):
                        if or_characterstic_tuple in ranked_hypothesis.keys():
                            if or_characterstic_tuple in hypothesis_occurrence_count:
                                ...
                                hypothesis_occurrence_count[or_characterstic_tuple] = 1
                            ranked_hypothesis[or_characterstic_tuple] -= ((weighted_property_dict[or_tuple[0]]+weighted_property_dict[or_tuple[1]])/2)*hypothesis_occurrence_count[or_characterstic_tuple]
